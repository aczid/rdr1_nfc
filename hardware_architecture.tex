...
%This means that malicious use must be ruled out at both endpoints and the payment provider has the greatest stake in managing trust.
%A technical solution to the authentication problem could be achieved using public key cryptography.

%A payment provider
%TODO Leg uit hoe je met trust omgaat in electronisch bankieren

\textit{Integrated Circuit Cards} (ICCs), commonly known as smartcards or chip cards, are cards which contain an integrated circuit system.
They can be used to authenticate transactions by electronically signing them using a preconfigured secret known only to the issuer of the bank.
...
This has lead to the widespread adoption of smart cards in banking, following the EMV standard developed by Europay, Mastercard and Visa (EMV). %have jointly designed a standard for smart card interoperability to provide a secure means of transaction authentication, which has been largely adopted by the banking industry and is in widespread use.
...
The GSM standard already provides a mechanism for identification and authentication on the network by means of a smart card, albeit in a different form factor. %secret key stored on an \textit{Universal Integrated Circuit Card} (UICC) by the telco. %TODO referentie
...
%In fact, the SIM card remains property of the telco after distribution, much like a bank card or passport remain the property of their respective issuers. %TODO referentie
However, the current generation of mobile devices lack a common infrastructure for third parties to make use of such secure storage for their own applications.
